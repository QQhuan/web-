...
2. 跨域相关的请求头字段、响应头字段
   ...
   > 请求头字段：`Content-Type`、`crossDomain`、`withCredentials`
   > 响应头字段：`access-control-allow-credentials: true`、`access-control-allow-origin: https`、`Access-Control-Allow-Methods: GET`, `POST, PUT、Access-Control-Allow-Headers: "Content-Type"`